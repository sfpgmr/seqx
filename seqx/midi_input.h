...
namespace sf {
  ...
  {
    midi_input_error(const boost::uint32_t error) : error_id_(error),exception(get_error_string(error)){}
  ...
  inline void throw_midi_input_error(const boost::uint32_t error)
  {
  ...
  struct midi_input
  {
    ...
    typedef boost::signals2::signal<void (boost::uint32_t wMsg,boost::uint32_t midi_message,boost::uint32_t time_stamp) > midi_message_arrived_t;
    midi_message_arrived_t midi_message_arrived;


    /** MIDI IN インターフェース*/
    struct caps {
      caps(const MIDIINCAPS2& value,const boost::uint32_t dev_id) :id_(dev_id),name_(value.szPname),device_info_(value)
      {
...
//      memset(control_change_buffer_,0,sizeof(boost::uint32_t) * midi_ch_num * control_change_num );
      ...
      boost::uint32_t result = midiInStart(hmidiin_);
      assert(result == MMSYSERR_NOERROR);
    ...
    void stop()
    {
      ...
      assert(hmidiin_ != NULL);
      boost::uint32_t result = midiInStop(hmidiin_);
      assert(result == MMSYSERR_NOERROR);
      throw_midi_input_error(result);
    }
    ...
    void reset()
    {
      if(hmidiin_ == NULL) return;
      assert(hmidiin_ != NULL);
      boost::uint32_t result = midiInReset(hmidiin_);
      assert(result == MMSYSERR_NOERROR);
      throw_midi_input_error(result);
    }
    ...
    void close()
    {
      ...
      boost::uint32_t result = midiInClose(hmidiin_);
      assert(result == MMSYSERR_NOERROR);
      hmidiin_ = NULL;
      throw_midi_input_error(result);
